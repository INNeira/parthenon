...
void SetInOrOut(std::shared_ptr<MeshBlockData<Real>> &rc) {
  ...
  bool const use_sparse = pmb->packages["calculate_pi"]->Param<bool>("use_sparse");
  if (use_sparse) {
...
std::shared_ptr<StateDescriptor> Initialize(ParameterInput *pin) {
  ...
  // add a variable called in_or_out that will hold the value of the indicator
  // function
  Metadata m({Metadata::Cell, Metadata::Derived});
  if (use_sparse) {
    m.Set(Metadata::Sparse);
  }
  package->AddField("in_or_out", m, DerivedOwnership::unique);

  // All the package FillDerived and CheckRefinement functions are called by parthenon
  package->FillDerived = SetInOrOut;
  // could use package specific refinement tagging routine (see advection example),
  // but instead this example will make use of the parthenon shipped first derivative
